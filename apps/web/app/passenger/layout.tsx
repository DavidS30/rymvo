...
async function PassengerGate({ children }: { children: React.ReactNode }) {
    ...
    return (
      <div className="rymvo-shell flex min-h-screen items-center justify-center">
        <div className="rymvo-card max-w-md p-8 text-center">
          <p className="rymvo-eyebrow">Rymvo</p>
          <h2 className="mt-4 text-xl font-bold text-[#d9a84e]">Acceso denegado</h2>
          <p className="mt-2 text-white/50">Solo pasajeros pueden acceder a esta sección.</p>
      ...
      <div className="rymvo-shell">
        <nav className="rymvo-topbar flex items-center justify-between border-b px-6 py-4 lg:px-10">
          ...
          <div className="flex items-center gap-4">
            <LanguageToggle /><ThemeToggle /><span className="hidden text-xs uppercase tracking-[.2em] text-white/40 sm:block">Passenger</span>
            <Link href="/passenger/book" className="text-xs font-semibold uppercase tracking-[.16em] text-[#d9a84e]">
              Reservar
            </Link>
            <UserButton />
          </div>
