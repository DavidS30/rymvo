...
import { formatCentsToDisplay, escapeHtml } from "../utils";
...
export async function sendBookingConfirmation(
  ...
  if (!resend) {
    console.log("[Email] Booking confirmation skipped (no Resend key configured)");
...
export async function sendDriverAssignment(
  ...
  if (!resend) {
    console.log("[Email] Driver assignment skipped (no Resend key configured)");
    console.log("  Booking:", input.bookingId);
    return;
  }
...
<body style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px">
  ...
  <div style="border:1px solid #e5e5e5;border-top:0;padding:24px;border-radius:0 0 8px 8px">
    <p style="font-size:16px">Hola <strong>${escapeHtml(input.passengerName)}</strong>,</p>
    <p>Tu reserva ha sido confirmada. Aquí están los detalles:</p>

    <table style="width:100%;border-collapse:collapse;margin:16px 0">
      <tr>
        <td style="padding:8px 0;color:#666">Servicio</td>
        <td style="padding:8px 0;font-weight:bold">${escapeHtml(serviceLabels[input.serviceType] ?? input.serviceType)}</td>
      </tr>
      <tr>
        <td style="padding:8px 0;color:#666">Fecha</td>
        <td style="padding:8px 0;font-weight:bold">${escapeHtml(date)}</td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td style="padding:8px 0;font-weight:bold">${escapeHtml(input.originAddress)}</td>
      </tr>
      <tr>
        <td style="padding:8px 0;color:#666">Destino</td>
        <td style="padding:8px 0;font-weight:bold">${escapeHtml(input.destAddress)}</td>
      </tr>
    ...
    <p style="margin-top:24px;font-size:14px;color:#888">Reserva #${escapeHtml(input.bookingId)}</p>
...
<body style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px">
  ...
  <div style="border:1px solid #e5e5e5;border-top:0;padding:24px;border-radius:0 0 8px 8px">
    <p style="font-size:16px">Hola <strong>${escapeHtml(input.driverName)}</strong>,</p>
    <p>Se te ha asignado un nuevo viaje:</p>

    <table style="width:100%;border-collapse:collapse;margin:16px 0">
      <tr>
        <td style="padding:8px 0;color:#666">Pasajero</td>
        <td style="padding:8px 0;font-weight:bold">${escapeHtml(input.passengerName)}</td>
      </tr>
      <tr>
        <td style="padding:8px 0;color:#666">Servicio</td>
        <td style="padding:8px 0;font-weight:bold">${escapeHtml(serviceLabels[input.serviceType] ?? input.serviceType)}</td>
